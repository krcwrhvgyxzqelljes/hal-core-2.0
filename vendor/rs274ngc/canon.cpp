...
std::vector<rs274ngc_data> _datavec;
#define AA
#ifdef AA
        ...
        )
{
    ...
    rs274ngc_data data;
    data.type=0;
    data.p0={_program_position_x,_program_position_y,_program_position_z};
    data.p1={x,y,z};
    data.abc0={_program_position_a,_program_position_b,_program_position_c};
    data.abc1={a,b,c};
    data.feed=_feed_rate;
    data.line_nr=_line_number;
    ...
    std::cout<<"N"<<_line_number<<" G0 X:"<<x<<" Y:"<<y<<" Z:"<<z<<" A:"<<a<<" B:"<<b<<" C:"<<c<<" F:"<<_feed_rate<<std::endl;
}
        ...
        )
{
    ...
    // Arc endpoint.
    data.p1={_program_position_x,_program_position_y,_program_position_z};
    data.pc={first_axis,second_axis,_program_position_z};
    ...
    data.feed=_feed_rate;
    data.line_nr=_line_number;
    data.plane=_active_plane;
     _datavec.push_back(data);

    std::cout<<"N"<<_line_number<<" G"<<g_nr<<" X:"<<first_end<<" Y:"<<second_end<<" Z:"<<axis_end_point<<
               " Cx"<<first_axis<<" Cy:"<<second_axis<<" A:"<<a<<" B:"<<b<<" C:"<<c<<" F:"<<_feed_rate<<std::endl;
}
        ...
        )
{
    ...
    std::cout<<"N"<<_line_number<<" G1 X:"<<x<<" Y:"<<y<<" Z:"<<z<<" A:"<<a<<" B:"<<b<<" c:"<<c<<" F:"<<_feed_rate<<std::endl;
}
...
void GET_EXTERNAL_PARAMETER_FILE_NAME(std::string file_name,                                 /* string: to copy file name into       */
                                      int max_size)                                     /* maximum number of characters to copy */
{
    _parameter_file_name=file_name;
}
